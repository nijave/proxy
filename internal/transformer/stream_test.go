...
func TestProxyResponsesStream_ToolOnlyStream(t *testing.T) {
	...
	// Expected: message_start, tool_start(0), input_json_delta,
	// tool_stop(0), message_delta(tool_use + usage), message_stop = 6.
	// Argument deltas are buffered upstream-side and flushed once at
	// output_item.done so whitespace-suffixed keys can be repaired.
	if len(events) != 6 {
		t.Fatalf("expected 6 events, got %d: %+v", len(events), events)
	}
	if events[0].Type != "message_start" {
	...
	if md.Type != "message_delta" || md.Delta == nil || md.Delta.StopReason != "tool_use" {
		t.Fatalf("event[4] = %+v, want message_delta(tool_use)", md)
	}
